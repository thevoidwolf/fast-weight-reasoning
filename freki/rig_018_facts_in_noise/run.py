"""rig_012 — Shared-bus cleanup + symmetric-consistency aux (Fable's fix).

Same recipe as rig_010 v2 (cleanup+aux+curriculum, aux always on, contrastive
mask), but the model uses SharedBusMixer instead of CleanupMixer, and we add
a symmetric-consistency aux term via mixer._last_sym_logits.

Diagnostic outcomes on n_entities=512 d_key=64:
  k4 ≥ 0.7  → H1+H6 confirmed (shared bus + sym aux fix decoupling); ship.
  aux converges, k4 chance → shared bus not enough; disease is deeper
                              (probably crosstalk in flat M; try DeltaNet write).
  aux fails to converge → role-typing needed (bridge tokens as separate
                          typed sub-vocab).
"""
...
sys.path.insert(0, str(HERE_PARENT / "rig_013_delta_write"))

from common.harness import RunCfg, cosine_lr, left_pad
from common.tasks import TaskCfg, N_CONTROL
from common.tasks_noise import (
    sample_3hop_facts_in_noise,
    sample_3hop_facts_in_noise_identity,
    sample_3hop_facts_in_filler_vocab,
    sample_3hop_facts_in_filler_vocab_identity,
    sample_3hop_facts_with_rel,
    sample_3hop_facts_with_rel_identity,
)
from model import DeltaBusConfig as SharedBusConfig, build_model
import torch


def _eval_ladder_noise(model, task_cfg, K1_max, K2, K3, n_noise,
                       batch, n_batches, device, eval_seed, pad_seq_to=0,
                       task_variant: str = "vocab_noise", filler_count: int = 32,
    ...
    for K1 in range(1, K1_max + 1):
        ...
        for _ in range(n_batches):
            if task_variant == "filler_vocab":
                seq, tgt, *_ = sample_3hop_facts_in_filler_vocab(
                    K1, k2_use, k3_use, task_cfg, batch, gen,
                    n_noise=n_noise, filler_count=filler_count, device=device)
            elif task_variant in ("rel_filler_vocab", "rel_value_vocab"):
                noise_src = "filler_vocab" if task_variant == "rel_filler_vocab" else "value_vocab"
                seq, tgt, *_ = sample_3hop_facts_with_rel(
                    K1, k2_use, k3_use, task_cfg, batch, gen,
                    n_noise=n_noise, n_rel_per_bank=n_rel_per_bank,
                    noise_source=noise_src, filler_count=filler_count,
                    device=device)
            else:
                seq, tgt, *_ = sample_3hop_facts_in_noise(
                    K1, k2_use, k3_use, task_cfg, batch, gen,
                    n_noise=n_noise, device=device, exclude_in_context=True)
            if pad_seq_to > 0:
...
def train_one_shared_bus(
    ...
    log(f"[{tag}] params={n_params:,} tasks={train_tasks} steps={cfg.steps} "
        ...
        f"λ_init={aux_lambda_init}, τ={aux_decay_tau}) sym(λ={sym_lambda_init}) "
        f"n_ent={cfg_n_entities} d_key={hs_cfg.d_key} contrastive={contrastive_aux}")
    curve = []
    start = time.time()
    ...
    for step in range(cfg.steps):
        ...
        if task_variant == "filler_vocab":
            sampler = sample_3hop_facts_in_filler_vocab if use_real else sample_3hop_facts_in_filler_vocab_identity
            seq, tgt, bridge1, bridge2 = sampler(
                k1r, k2_use, k3_use, task_cfg, cfg.batch, train_gens[tk],
                n_noise=n_noise, filler_count=filler_count, device=cfg.device)
        elif task_variant in ("rel_filler_vocab", "rel_value_vocab"):
            noise_src = "filler_vocab" if task_variant == "rel_filler_vocab" else "value_vocab"
            sampler = sample_3hop_facts_with_rel if use_real else sample_3hop_facts_with_rel_identity
            seq, tgt, bridge1, bridge2 = sampler(
                k1r, k2_use, k3_use, task_cfg, cfg.batch, train_gens[tk],
                n_noise=n_noise, n_rel_per_bank=n_rel_per_bank,
                noise_source=noise_src, filler_count=filler_count,
            ...
            sampler = sample_3hop_facts_in_noise if use_real else sample_3hop_facts_in_noise_identity
            if use_real:
                seq, tgt, bridge1, bridge2 = sampler(
                    k1r, k2_use, k3_use, task_cfg, cfg.batch, train_gens[tk],
                    n_noise=n_noise, exclude_in_context=True, device=cfg.device)
            else:
                seq, tgt, bridge1, bridge2 = sampler(
                    k1r, k2_use, k3_use, task_cfg, cfg.batch, train_gens[tk],
                    n_noise=n_noise, device=cfg.device)
        bridge3 = None
        if cfg.pad_seq_to > 0:
        ...
        if (step + 1) % cfg.eval_every == 0 or (step + 1) == cfg.steps:
            ...
            accs = _eval_ladder_noise(model, task_cfg, eval_K1_max, cfg.K2,
                                      cfg.K3, n_noise,
    ...
    payload = {
        "tag": tag, "rig": "rig013_delta_write", "seed": seed,
...
def main():
    ...
    ap.add_argument("--n-noise", type=int, default=8,
                    help="Random tokens inserted between each fact pair.")
    ap.add_argument("--task-variant", choices=("vocab_noise", "filler_vocab",
                                               "rel_filler_vocab", "rel_value_vocab"),
                    default="vocab_noise",
                    help="'vocab_noise': noise from full vocab (excluding in-context). "
                         "'filler_vocab': noise from a disjoint filler-only vocab range. "
                         "'rel_filler_vocab': cue-ladder (b) — REL token + filler-vocab noise. "
                         "'rel_value_vocab': cue-ladder (b) hard — REL token + value-vocab noise.")
    ap.add_argument("--n-rel-per-bank", type=int, default=1,
                    help="1 = single REL per bank. >1 = paraphrased relations (cue-ladder c).")
    ap.add_argument("--filler-count", type=int, default=32,
                    help="For filler_vocab variant: size of dedicated filler-token pool.")
    ap.add_argument("--use-protect-gate", action="store_true", default=False,
                    help="Enable memory-aware β gate (Fable's fix): "
                         "β_t = σ(logit - w_p * ||M^T k||^2 / d).")
    ap.add_argument("--protect-w-init", type=float, default=0.1,
                    help="Initial value of learnable w_p in the protect gate.")
    ap.add_argument("--tag-prefix", default="rig018_facts_in_noise")
    ...
    hs_cfg = SharedBusConfig(
        ...
        use_protect_gate=args.use_protect_gate,
        protect_w_init=args.protect_w_init,
    )
    ...
    for seed in args.seeds:
        payload = train_one_shared_bus(
            ...
            n_rel_per_bank=args.n_rel_per_bank,
        )
        per_seed.append(payload)

    summary = {
        "rig": "rig013_delta_write",
        "seeds": args.seeds,
