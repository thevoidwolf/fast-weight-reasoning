"""rig_006_residual_reads — FREKI variant with residual (summed) reads.

See model.py for the ResRead design rationale: swap the K_chain=3 chained
final `y = RMSNorm(M·RMSNorm(M·RMSNorm(M·q)))` for a sum of per-depth reads
`y = sum_h RMSNorm_h(M · y_{h-1})`, keeping every other piece of FREKI
identical. Tests whether the K_chain=3 depth wall is really about the
AND-condition on the read output.
"""
...
from model import ResReadConfig, build_model
...
def main():
    ...
    ap.add_argument("--tag-prefix", default="rig006_resread")
    ...
    rr_cfg = ResReadConfig(
        d_model=args.d_model, d_key=args.d_key, n_layers=args.n_layers,
        beta_bias_init=args.beta_bias_init, K_chain=args.K_chain,
    )
    out_dir = HERE / "outputs"

    def make_builder(rr_cfg):
        return lambda vocab_size: build_model(vocab_size, rr_cfg)

    per_seed = []
    for seed in args.seeds:
        payload = train_one(
            build_model=make_builder(rr_cfg),
            rig_name="rig006_resread", seed=seed, cfg=cfg,
